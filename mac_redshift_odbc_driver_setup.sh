...
export odbc_dir=$HOME/odbc2

echo $odbc_dir

mkdir -p $odbc_dir

# Not going to install if directory already exists
if [ -d "/opt/amazon/redshift" ]; then
  return
fi
...
export DYLD_LIBRARY_PATH=/opt/amazon/redshift/lib/universal
...
echo "-----------------------"
echo "IMPORTANT"
echo "$HOME/odbc/envvars.sh needs to be sourced to enable redshift odbc driver to work"
echo "~~~~~~~~~~~~~~~~~~~~~~~"
